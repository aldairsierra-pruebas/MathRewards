...
async function saveAttempt(payload) {
  ...
  await setDoc(doc(db, 'players', playerId, 'sessions', sessionId, 'attempts', attemptId), {
    ...
    itemDifficulty: payload.itemDifficulty || payload.difficultyScore || 1,
    itemSkillProfile: payload.itemSkillProfile || payload.skillProfile || null,
    masteryScoreBefore: payload.masteryScoreBefore ?? null,
    masteryScoreAfter: payload.masteryScoreAfter ?? null,
    recommendedTier: payload.recommendedTier ?? null,
